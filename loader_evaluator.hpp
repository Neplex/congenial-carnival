...
/*!
 * \file
 * This module provides a class to read (from a stream) an expression an store
 * it for evaluation.
 *
...
/*!
 * This class is used to read an expression in postfix form on an input stream
 * and then to evaluate it at will.
 */
...
public:
  ...
  /*!
   * Evaluate the held expression starting from an empty
   * Evaluation_Context_Simple.
   * \param in \c istream to read the value from.
   * \return The computed value.
   */
  double evaluate(std::istream &in);
  /*! Destructor. */
  ~Loader_Evaluator() { delete expression; };
};
