...
/*!
 * \file
 * This module provides the classes to represent and use variables in
...
public:
  // CONSTRUCTORS
  Variable(std::string const &_id) : Expr(priority_var_con), id(_id) {
    assert("" != _id);
  };
  /*! Destructor. */
  ~Variable(){};
...
public:
  // CONSTRUCTOR
  Set(Variable *variable, Expr *value)
      : Expr(priority_set), variable(variable), value(value) {
