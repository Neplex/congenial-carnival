...
/*!
 * \file
 * This module provides the classes to represent binary arithmetical
 * expressions.
 *
...
class Op_Binary : public Expr {
  ...
  std::string sign;
  /*! The expression on the left of the of the operator */
  Expr *left;
  /*! The expression on the right of the of the operator */
  Expr *right;

public:
  // CONSTRUCTOR
  Op_Binary(priority_enum priority, std::string sign, Expr *left, Expr *right)
      : Expr(priority), sign(sign), left(left), right(right){};
  /*! Destructor. */
  ~Op_Binary() {
    delete left;
    delete right;
...
protected:
  /*! ??? */
  virtual double compute(double left, double right) const = 0;
};
...
#define CLASS_OP_BINARY(name, priority, sign)                                  \
  ...
  public:                                                                      \
    name(Expr *exp1, Expr *exp2) : Op_Binary(priority, sign, exp1, exp2){};    \
    /*! Destructor. */                                                         \
    ~name(){};                                                                 \
                                                                               \
  protected:                                                                   \
    double compute(double left, double right) const;                           \
  };
